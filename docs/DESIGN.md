...
| **H2 sequence allocator concurrent-safe** | ✅ Done | `LedgerSequenceAllocator` dialect split eliminated. Both H2 2.2+ (in `MODE=PostgreSQL`) and PostgreSQL use identical SQL: `INSERT ON CONFLICT DO NOTHING + UPDATE`. H2's MVStore (MVCC) made the old `MERGE INTO` non-atomic for concurrent first-inserts (race on `WHEN NOT MATCHED` → PK violation). `JpaSequenceNumberConcurrentH2IT` (5-thread) added. Unblocks casehubio/aml CI. Closes #148. |
