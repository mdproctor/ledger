...
| **Bilateral entry signing** | ✅ Done | `AgentKeyProvider` SPI + `ConfiguredAgentKeyProvider` (@DefaultBean, per-actorId PKCS#8/X.509 PEM config); `AgentSignatureEnricher` (LedgerEntryEnricher, signs `canonicalBytes()` at @PrePersist); `LedgerEntry.agentSignature` + `agentPublicKey` BYTEA fields (V1005); `LedgerVerificationService.verifyAgentSignature()` (UNSIGNED/VALID/INVALID, self-contained from stored public key bytes); `VerificationResult` enum; ADR 0011. 405 tests. Closes #79. |
